({
    ...
    getContactFromMaster : function(cmp, accountRecord){
        //once we've identified the right account, lets go get all the infos for adding to sf
        console.log("getting master contact detail from " + accountRecord.part_accountNumber__c);
		var action = cmp.get("c.masterGetContactByAcctNumber");
        action.setParams({accountNumber: accountRecord.part_accountNumber__c});
    ...
    createRecordsFromMaster : function(cmp, accountRecord, contactRecord){
        ...
        console.log("creating customer data " + accountRecord.part_accountNumber__c);
        console.log("account");
        ...
        action.setCallback(this, function(response){
            ...
            if(state === "SUCCESS"){
                ...
                window.location.replace('https://cdg-part-2-dev-ed.lightning.force.com/lightning/r/Case/' + retRows + '/view');
            }else if(state === "INCOMPLETE"){
                console.log("this isn't good");
            }else if(state === "ERROR"){
                var errors = response.getError();
